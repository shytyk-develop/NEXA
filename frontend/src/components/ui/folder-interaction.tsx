...
/** Front flap outline (Figma export, viewBox 0 0 235 121). */
const FLAP_PATH =
...
function FolderInteraction({
    ...
    // Per-instance SVG ids (clip + gradients), safe in url(#…)
    const uid = useId().replace(/[^a-zA-Z0-9_-]/g, '');
    const ids = { clip: `folder-flap-clip-${uid}` };
        ...
        >
            <div className="w-80 h-52 relative wrapper folder-interaction__wrapper">
                ...
                >
                    ...
                    >
                        {/* Clear flap: a light blur clipped to the flap shape, an almost
                            transparent tint and one hairline — the sheet reads through it. */}
                        <foreignObject x="-13" y="-13" width="262.4" height="148.4">
                            <div
                                className="folder-interaction__flap-blur"
                                style={{ clipPath: `url(#${ids.clip})` }}
                            />
                        </foreignObject>
                        <path
                            className="folder-interaction__flap-shape"
                            d={FLAP_PATH}
                            vectorEffect="non-scaling-stroke"
                        />
                        <defs>
                            <clipPath id={ids.clip} transform="translate(13 13)">
                                <path d={FLAP_PATH} />
                            </clipPath>
                        </defs>
                    </svg>
