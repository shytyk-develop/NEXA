/**
 * Nexa level slider — adapted from ClaudeModelSelector (web component).
 * Discrete levels with spring snap, ultra track effect, and inline settings mode.
 */

export const LEVELS = ['Low', 'Medium', 'High', 'Extra', 'Max', 'Ultracode'];
/** Preference percents mapped to each level index (0–5). */
export const LEVEL_PERCENTS = [0, 20, 40, 60, 80, 100];
...
export function levelIndexToPercent(index) {
    const i = Math.min(LEVELS.length - 1, Math.max(0, Math.round(Number(index) || 0)));
    return LEVEL_PERCENTS[i];
}
...
/** Spark / ultra-track palettes keyed to wallpaper packs in app-layout.css. */
const WALLPAPER_SPARK_SEEDS = {
    mist: { deep: [88, 98, 118], mid: [130, 140, 160], bright: [175, 185, 205] },
    ember: { deep: [140, 70, 40], mid: [185, 110, 65], bright: [230, 165, 110] },
    aurora: { deep: [40, 130, 185], mid: [90, 170, 220], bright: [170, 155, 235] },
    tide: { deep: [30, 80, 190], mid: [40, 145, 220], bright: [100, 195, 240] },
    sage: { deep: [50, 100, 72], mid: [85, 140, 105], bright: [140, 185, 150] },
    cobalt: { deep: [40, 65, 150], mid: [70, 105, 195], bright: [120, 155, 230] },
    rose: { deep: [150, 60, 90], mid: [195, 105, 135], bright: [235, 160, 180] },
    dusk: { deep: [100, 65, 160], mid: [140, 110, 195], bright: [185, 160, 230] },
    graphite: { deep: [90, 94, 104], mid: [135, 140, 150], bright: [185, 190, 200] },
    void: { deep: [55, 58, 68], mid: [95, 100, 115], bright: [145, 150, 165] },
};

function buildUltraPalette(wallpaperId) {
    const seed = WALLPAPER_SPARK_SEEDS[wallpaperId] || WALLPAPER_SPARK_SEEDS.mist;
    const { deep, mid, bright } = seed;
    const soft = mixRgb(mid, bright, 0.45);
    const pale = mixRgb(bright, [235, 235, 242], 0.35);
    const highlight = mixRgb(bright, [245, 245, 250], 0.55);
    const peak = mixRgb(bright, [255, 255, 255], 0.72);
    return {
        id: WALLPAPER_SPARK_SEEDS[wallpaperId] ? wallpaperId : 'mist',
        left: [58, 58, 66],
        deep,
        mid,
        bright,
        tones: [deep, deep, mid, mid, mid, soft, soft, bright, pale],
        highlight,
        peak,
        accent: toCssRgb(bright),
        accentDeep: toCssRgb(mid),
    };
}

function readActiveWallpaperId() {
    return document.getElementById('page-chat')?.dataset?.wallpaper || 'mist';
}

let instanceCount = 0;

class NexaLevelSlider extends HTMLElement {
    static get observedAttributes() {
        return ['value', 'open', 'disabled', 'inline', 'label', 'hint', 'axis-start', 'axis-end'];
    }

    constructor() {
        super();
        this.attachShadow({ mode: 'open' });
        this._uid = `nexa-level-${++instanceCount}`;
        this._value = 0;
        this._levelIndex = 0;
        ...
        this._palette = buildUltraPalette('mist');
        this._wallpaperObserver = null;
      ...
      <style>
        :host {
          ...
          --effort-accent-deep: #c4b5fd;
          --effort-text: #a1a1aa;
          --effort-text-strong: #f4f4f5;
          --effort-muted: #8a8a8a;
          --effort-track: color-mix(in srgb, #fff 8%, transparent);
          --effort-track-fill: color-mix(in srgb, #fff 14%, transparent);
          --effort-progress: 0;
          --effort-thumb-w: 1.5rem;
          --effort-thumb-h: 1.625rem;
          --effort-track-pad: 1px;
          --effort-track-radius: 0.625rem;
          --effort-surface: color-mix(in srgb, #fff 4.5%, transparent);
          --effort-outline: color-mix(in srgb, #fff 6%, transparent);
          --effort-width: 100%;
          --effort-ultra-a: color-mix(in srgb, var(--effort-accent) 18%, transparent);
          --effort-ultra-b: color-mix(in srgb, var(--effort-accent) 35%, transparent);
          --effort-ultra-c: color-mix(in srgb, var(--effort-accent-deep) 45%, transparent);
          --ease-decay: cubic-bezier(0.2, 0, 0, 1);
          display: block;
          width: var(--effort-width);
          max-width: 100%;
          color: var(--effort-text);
          font-family: var(--f-ui, ui-sans-serif, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif);
          font-size: 1rem;
          line-height: 1.4;
          font-synthesis: none;
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        :host(:not([open]):not([data-closing])) .panel { display: none; }
        ...
        :host([inline]) .shell { min-height: 0; }
        :host([inline]) .anchor-row { display: none; }
        :host([inline]) .panel {
        ...
        :host([inline][data-closing]) .panel { opacity: 1; transform: none; }
        ...
        .level-current { transition-delay: 24ms; }
        ...
        .level-outgoing { pointer-events: none; transition-delay: 0ms; }
        .level-outgoing.is-exiting {
          opacity: 0;
          transform: translateY(var(--label-exit-y, -3px));
          filter: blur(2px);
        }
        :host([data-ultra]) .level-current { color: var(--effort-accent); }
        ...
        :host([data-ultra]) .track-fill { opacity: 0; }
        ...
        .track::before {
          ...
          background: linear-gradient(
            ...
            var(--effort-ultra-a) 32%,
            var(--effort-ultra-b) 58%,
            var(--effort-ultra-c) 100%
          );
          opacity: 0;
          transition-property: opacity;
        ...
        :host([data-ultra]) .track::before { opacity: 1; }
        ...
        .ultra-fallback {
          background: linear-gradient(
            90deg,
            color-mix(in srgb, #fff 8%, transparent) 0%,
            color-mix(in srgb, var(--effort-accent) 22%, transparent) 40%,
            color-mix(in srgb, var(--effort-accent-deep) 40%, transparent) 100%
          );
        }
        :host([data-ultra][data-pixels-ready]) .pixel-field { opacity: 1; }
        ...
        .tick:last-child { background: var(--effort-accent); opacity: 1; }
        :host([data-ultra]) .tick { opacity: 0; }
        ...
        .range::-webkit-slider-thumb {
          ...
          border-radius: 0.5rem;
          background: #f4f4f5;
          box-shadow: 0 1px 2px rgba(0, 0, 0, 0.35), 0 4px 10px rgba(0, 0, 0, 0.2);
          cursor: ew-resize;
        ...
        .range:active::-moz-range-thumb { transform: scale(0.96); }
        ...
        .trigger:hover { background: color-mix(in srgb, #fff 11%, transparent); }
        .trigger:active { scale: 0.96; }
        .trigger-value { font-variant-numeric: tabular-nums; }

        @media (prefers-reduced-motion: reduce) {
          .level-current, .level-outgoing, .panel, .track::before, .track-fill,
          .range::-webkit-slider-thumb, .range::-moz-range-thumb, .trigger, .help-button, .tooltip {
            transition-duration: 0.001ms;
          }
          :host([data-ultra]) .ultra-fallback { opacity: 1; }
      ...
      <div class="shell">
        <div class="anchor-row">
          <button class="trigger" type="button" aria-controls="${this._uid}-panel" aria-expanded="false" aria-label="Level: Low">
            <span class="trigger-value">Low</span>
          </button>
        </div>

        <section class="panel" id="${this._uid}-panel" aria-label="Level settings">
          <div class="header">
            <div class="title">
              <span class="title-label">Level</span>
            ...
            <div class="help-wrap">
              <button class="help-button" type="button" aria-label="About levels" aria-describedby="${this._uid}-tooltip">
              ...
              <div class="tooltip" id="${this._uid}-tooltip" role="tooltip">
                Higher levels increase intensity. Ultracode is the strongest setting.
              </div>
            </div>
          </div>

          <div class="axis" aria-hidden="true">
            <span class="axis-start">Lower</span>
            <span class="axis-end">Higher</span>
          </div>
          ...
          <div class="track-shell">
            <div class="track" aria-hidden="true">
              ...
              <div class="ticks">${LEVELS.map(() => '<span class="tick"></span>').join('')}</div>
            </div>
            <input class="range" type="range" min="0" max="5" step="0.001" value="0" aria-label="Level" aria-valuemin="0" aria-valuemax="5" aria-valuetext="Low" />
          </div>
        </section>
      </div>
    `;
        ...
        this._titleLabel = this.shadowRoot.querySelector('.title-label');
        this._tooltip = this.shadowRoot.querySelector('.tooltip');
        this._axisStart = this.shadowRoot.querySelector('.axis-start');
        this._axisEnd = this.shadowRoot.querySelector('.axis-end');

        this._onDocumentPointerDown = this._onDocumentPointerDown.bind(this);
        this._onReducedMotionChange = this._onReducedMotionChange.bind(this);
        this._onWallpaperAttributeChange = this._onWallpaperAttributeChange.bind(this);
    }
    ...
    connectedCallback() {
        ...
        this._syncCopyFromAttributes();
        this._syncWallpaperPalette(readActiveWallpaperId());
        const initialValue = Number.parseFloat(this.getAttribute('value') ?? '0');
        this._setValue(Number.isFinite(initialValue) ? initialValue : 0, {
            animateLabel: false,
            reflect: false,
        });
        if (this.hasAttribute('inline') && !this.open) this.setAttribute('open', '');
        this._syncOpenState();
    ...
    attributeChangedCallback(name, oldValue, newValue) {
        ...
        if (['label', 'hint', 'axis-start', 'axis-end'].includes(name)) {
            this._syncCopyFromAttributes();
            return;
        }
        ...
        if (name === 'inline' && this.isConnected) {
            if (this.hasAttribute('inline') && !this.open) this.setAttribute('open', '');
            this._syncOpenState();
        }
    }

    _syncCopyFromAttributes() {
        if (this._titleLabel) this._titleLabel.textContent = this.getAttribute('label') || 'Level';
        if (this._tooltip) {
            this._tooltip.textContent =
                this.getAttribute('hint') ||
                'Higher levels increase intensity. Ultracode is the strongest setting.';
        }
        if (this._axisStart) this._axisStart.textContent = this.getAttribute('axis-start') || 'Lower';
        if (this._axisEnd) this._axisEnd.textContent = this.getAttribute('axis-end') || 'Higher';
    }
    ...
    _observeWallpaper() {
        ...
        if (!pageChat) return;
        this._wallpaperObserver = new MutationObserver(this._onWallpaperAttributeChange);
        this._wallpaperObserver.observe(pageChat, {
            attributes: true,
            attributeFilter: ['data-wallpaper'],
        });
    }

    _onWallpaperAttributeChange(mutations) {
        for (const mutation of mutations) {
            if (mutation.attributeName !== 'data-wallpaper') continue;
            this._syncWallpaperPalette(readActiveWallpaperId());
            if (this._isUltra) this._drawPixelField(performance.now());
        }
    }

    _syncWallpaperPalette(wallpaperId) {
        const palette = buildUltraPalette(wallpaperId);
        this._palette = palette;
        this.style.setProperty('--effort-accent', palette.accent);
        this.style.setProperty('--effort-accent-deep', palette.accentDeep);
    }
    ...
    toggle() {
        if (this.disabled || this.hasAttribute('inline')) return;
        this.open ? this.close() : this.openPanel();
    }

    _syncOpenState() {
        const isOpen = this.open || this.hasAttribute('inline');
    ...
    _setValue(nextValue, { animateLabel = true, reflect = false } = {}) {
        ...
        this.style.setProperty('--effort-progress', String(safeValue / (LEVELS.length - 1)));
        ...
        const labelPrefix = this.getAttribute('label') || 'Level';
        this._triggerValue.textContent = LEVELS[nextIndex];
        this._trigger.setAttribute('aria-label', `${labelPrefix}: ${LEVELS[nextIndex]}`);
    ...
    _drawPixelField(time) {
        ...
        const palette = this._palette || buildUltraPalette(readActiveWallpaperId());
        const leftColor = palette.left;
        const highlightColor = palette.highlight;
        const peakColor = palette.peak;
        ...
        for (let row = 0; row < rows; row += 1) {
            for (let column = 0; column < columns; column += 1) {
                ...
                const cycleHash =
                    Math.abs(Math.sin(column * 17.17 + row * 41.73 + cycle * 13.11) * 24634.6345) % 1;
                const widthHash =
                    Math.abs(Math.sin(column * 5.37 + row * 29.11 + cycle * 7.43) * 17391.443) % 1;
                ...
                const flowHashA =
                    Math.abs(Math.sin(flowIndex * 18.31 + row * 37.17) * 19283.173) % 1;
                const flowHashB =
                    Math.abs(Math.sin((flowIndex + 1) * 18.31 + row * 37.17) * 19283.173) % 1;
                ...
                const variedTone = [
                    clamp(cellTone[0] + chromaNudge * 0.35 - depthBias * 8, 40, 245),
                    clamp(cellTone[1] - depthBias * 10 + (baseHash - 0.5) * 8, 35, 245),
                    clamp(cellTone[2] + depthBias * 6 + (cycleHash - 0.5) * 6, 40, 245),
                ];
                const hueAmount = smoothstep(0.1, 0.88, normalizedX);
                const baseColor = [
                    mix(leftColor[0], variedTone[0], hueAmount),
                    mix(leftColor[1], variedTone[1], hueAmount),
                    mix(leftColor[2], variedTone[2], hueAmount),
                ];
        ...
        this.dispatchEvent(
            new CustomEvent(type, {
                ...
                detail: {
                    ...
                    value: this._value,
                    percent: levelIndexToPercent(this._levelIndex),
                },
            })
        );
    }
...
if (typeof customElements !== 'undefined' && !customElements.get('nexa-level-slider')) {
    customElements.define('nexa-level-slider', NexaLevelSlider);
}

export { NexaLevelSlider };
