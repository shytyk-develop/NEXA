...
function buildEngineConfig(props, running) {
    const merged = {
        ...DEFAULTS,
        ...props,
    ...
    const line = merged.lineOptions;
    const dot = merged.dotOptions;
    const comet = merged.cometOptions;
    const shove = merged.repelOptions;

    return {
        floorRadius: merged.bottomRadius / PX_PER_WORLD,
        waistRadius: merged.waistRadius / PX_PER_WORLD,
        crownRadius: merged.topRadius / PX_PER_WORLD,
        waistAt: 1 - merged.waistPosition / 100,
        twist: merged.twist,
        zoom: merged.zoom,
        flowDir: merged.direction === 'left' ? -1 : 1,
        flowSpeed: (merged.speed / 100) * (merged.direction === 'left' ? -1 : 1),
        lineCount: line.count,
        lineColor: line.color,
        lineGlow: (line.glow / 10) * LINE_GLOW_MAX,
        showDots: merged.dots,
        dotCount: dot.count,
        dotSize: dot.size / DOT_SIZE_SCALE,
        dotColor: dot.color,
        dotGlow: (dot.glow / 10) * DOT_GLOW_MAX,
        dotFlicker: dot.flicker / 10,
        showComets: merged.comets,
...
function createVortex(canvas, container, cfgRef) {
    ...
    function build() {
        ...
        const strandMat = track(withRepel(new THREE.LineBasicMaterial({
            vertexColors: true,
            transparent: true,
            opacity: 0.5,
        ...
        if (dotCount > 0) {
            ...
            const dotMat = track(withRepel(new THREE.MeshBasicMaterial({
                color: 0xffffff,
                transparent: true,
                opacity: 0.9,
                blending: THREE.AdditiveBlending,
        ...
        for (let i = 0; i < cometTotal; i++) {
            ...
            const lineMat = track(withRepel(new THREE.LineBasicMaterial({
                vertexColors: true,
                transparent: true,
                opacity: 0.9,
                blending: THREE.AdditiveBlending,
            ...
            const headMat = track(new THREE.SpriteMaterial({
                map: cometTex,
                transparent: true,
                opacity: 0,
                blending: THREE.AdditiveBlending,
    ...
    const onPointerMove = (e) => {
        ...
        repelTarget = cfg.hoverRepel && cfg.running
    ...
    function drawStrand(strand, now, entrance) {
        ...
        for (let i = 0; i < STRAND_SEGMENTS; i++) {
            ...
            let edge = 1;
            if (u < FADE_ZONE) edge = (u / FADE_ZONE) ** 2;
            else if (u > 1 - FADE_ZONE) edge = ((1 - u) / FADE_ZONE) ** 2;
            let tip = 1;
            if (s > reach) tip = 0;
...
export function mountVortex(container, props = HERO_VORTEX_PRESET) {
    destroyVortex();
...
export function destroyVortex() {
    activeVortex?.dispose();
    activeVortex = null;
}
