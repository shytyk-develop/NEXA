...
import { areLinkWarningsEnabled } from './linkWarnings.js';
import { openMessageLinkBar } from './messageQuickBar.js';
...
/**
 * Intercept a message link click: open the inline hold-to-open warning in the
 * message's bubble, or go straight to the link while warnings are paused / off.
...
export function handleExternalLinkClick(event, anchor) {
    ...
    if (!areLinkWarningsEnabled() || !row) {
        window.open(safeHref, '_blank', 'noopener,noreferrer');
        return true;
    }
    openMessageLinkBar(row, safeHref);
    return true;
}
